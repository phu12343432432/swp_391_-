        ...
        <div class="min-h-screen bg-gray-100 flex items-center justify-center px-4">
            <div>
                <form class="bg-white shadow-lg rounded-lg p-8 max-w-4xl w-full" action="league" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start">
                        <div class="flex flex-col items-center text-center md:text-left md:items-start">
                            <h2 class="text-2xl text-gray-800 font-semibold mb-4">
                                <i class="fas fa-user-circle mr-2"></i>Chi tiết bài blog
                            </h2>
                            <c:choose>
                                <c:when test="${BLOG.image != null}">
                                    <img src="data:image/png;base64,${BLOG.image}" alt="Profile picture" id="profile-picture" class=" border-3 border-green-500 p-1 mb-3" style="width: 200px; cursor: pointer; margin: 10px auto;border: 2px solid #1b730d">
                                </c:when>
                                <c:otherwise>
                                    <img src="https://placehold.co/100x100" alt="Profile picture" id="profile-picture" class=" border-3 border-green-500 p-1 mb-3" style="width: 200px; cursor: pointer; margin: 10px auto;border: 2px solid #1b730d">
                                </c:otherwise>
                            </c:choose>
                        ...
                        <div class="mt-8 md:mt-0 md:ml-10 w-full max-w-lg">
                            ...
                            <div class="space-y-4" >
                                ...
                                <input type="hidden" name="leagueId" value="${BLOG.id}" />
                                <div>
                                    <label for="surname" class="text-gray-700">Blog title *</label>
                                    <input name="name" value="${BLOG.title}" type="text" id="surname"  class="w-full mt-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" readonly>
                                </div>
                    ...
                    <div>
                        <label for="desc" class="text-gray-700">Nội dụng*:</label>
                        <textarea name="desc" type="email" id="Email" style="height: 400px"
                                  ...
                                  readonly>${BLOG.description}</textarea>
                    </div>
                ...
                <c:forEach items="${COMMENTS}" var="comment">
                    <div class="row p-3" id="faq">
                        ...
                        <div class="col-md-11" style="display: flex; justify-content: space-between">
                            ...
                            <div>
                                <a href="blog-league?action=deleteComment&blogId=${BLOG.id}&id=${comment.id}&leagueId=${leagueId}" style="padding: 10px; cursor: pointer">X</a>
                            </div>
                ...
                <nav aria-label="Page navigation example" style="display: flex; justify-content:center;margin-top: 15px;">
                    <ul class="pagination">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <li class="page-item"><a class="page-link" href="blog-league?action=view&leagueId=${leagueId}&index=${selectedPage-1}&blogId=${BLOG.id}"><<</a></li>
                                </c:otherwise>
                            </c:choose>
                            <c:forEach var="i" begin="1" end="${endP}">
                            <li class="page-item ${i == selectedPage ? "active" : "" }"> <a class="page-link" href="blog-league?action=view&leagueId=${leagueId}&index=${i}&blogId=${BLOG.id}">${i}</a> <li>
                            </c:forEach>
                            <c:choose>
                ...
                <div class=" p-3">
                    <form action="blog-league" method="POST">
